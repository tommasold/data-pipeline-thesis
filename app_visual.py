...
def open_terminal(command):
    # Usa AppleScript per aprire un nuovo terminale e eseguire il comando
    full_command = f'tell app "Terminal" to do script "cd {project_path} && {command}"'
    subprocess.Popen(['osascript', '-e', full_command])

# Inserisci il percorso della tua cartella progetto
...
tk.Button(root, text="▶️ Macchina 1 (PostgreSQL)", width=40,
          command=lambda: open_terminal("python3 auto_1.py")).pack(pady=5)

tk.Button(root, text="▶️ Macchina 2 (PostgreSQL)", width=40,
          command=lambda: open_terminal("python3 auto_2.py")).pack(pady=5)

tk.Button(root, text="▶️ Macchina 3 (MQTT)", width=40,
...
tk.Button(root, text="📦 Esegui dbt run", width=40,
          command=lambda: open_terminal("python3 dbt.run")).pack(pady=5)
